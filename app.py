    ...
    if not st.session_state.chat_history:
        ...
        with col1:
            if st.button("📑 Summarize", use_container_width=True):
                suggested = "Summarize this document."
    
        with col2:
            if st.button("📖 Main Topics", use_container_width=True):
                suggested = "What are the main topics discussed in this document?"
    
        col3, col4 = st.columns(2)
    
        with col3:
            if st.button("🔑 Key Points", use_container_width=True):
                suggested = "What are the key points in this document?"
    
        with col4:
            if st.button("🧠 Explain Concepts", use_container_width=True):
                suggested = "Explain the important concepts in this document."
